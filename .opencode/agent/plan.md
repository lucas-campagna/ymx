...
- **Read the current state from `docs/` before discussing.** Read `docs/impl/README.md` (milestone table with Owner agent + Depends on, and the agent-workflow section) and the frontmatter (`status`, `depends_on`) of each `docs/impl/<version>-*.md` file. Read `docs/PRD.md` for the spec.
