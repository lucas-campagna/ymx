...
You execute a milestone **top-level task by top-level task** — each top-level `## Tasks` checkbox (together with its nested sub-bullets) is one unit of work and **one commit**.

1. **Propose.** Based on the plan, propose the next unblocked milestone. Read its **Crate(s)** column from the README table and map it to the specialist via the table above. Name the specialist you will spawn. Ask the user to confirm before spawning. Example: *"Next unblocked: milestone 1.3 (loading), crates `ymx-lib`,`ymx-core` -> specialist `loader`. Spawn it?"* Process **one milestone at a time** so toggles+commits stay clean and ordered; do not run two milestones' implementation phases in parallel (the propose/confirm phase may name multiple unblocked milestones, but implement serially).

2. **Dispatch (one specialist session per milestone).** On confirmation, spawn the owner specialist via the Task tool with a detailed task description and **keep its returned `task_id`** for the whole milestone. Point it at its `docs/impl/<version>-*.md` task checklist and the relevant `docs/PRD.md` sections. Give it these standing rules:
...
3. **Per task: verify, toggle, commit, resume.** When the specialist returns "task N done":
   ...
   (d) **Commit** — stage exactly the crate files this task touched plus the one docs toggle (run `git status` + `git diff --stat` first to identify them; add paths **explicitly** — never `git add -A`/`git add .`); never stage `docs/PRD.md`, `docs/impl/README.md`, `.opencode/`, or root manifests this task didn't touch. Message form: `feat(<version>): <task summary>` (derive a <=60-char summary from the task line, minus the marker). If no crate file changed (docs-only task), just commit the toggle.
   (e) **Resume** the specialist via its `task_id`: *"Task <N> committed. Implement task <N+1> now; stop and report when done."* Repeat (a)-(e) for each top-level task. If the specialist returns having completed more than one task in a single turn, toggle each such task's boxes and commit them together in one commit whose message lists the covered tasks (graceful fallback; ideal is one task per turn).
...
5. Close out — toggle acceptance, commit, tag, report. Once gatekeeper returns PASS:
   ...
   (d) **Report** done to the user and tell them: *"Invoke `/update` to flip milestone `<version>` frontmatter `status: done` and update the README table (committed as a separate docs change)."*

The tag marks implementation-verified completion (gatekeeper PASS); the frontmatter `status: done` flip + README table update is a `/update` follow-up, committed separately. You do **not** flip frontmatter `status`, edit `docs/impl/README.md`, or spawn `spec-curator`.
...
- Do not edit `docs/PRD.md`, `docs/impl/README.md`, `docs/impl/<version>-*.md` **frontmatter**, or the **text** of any task/acceptance line. The **only** edit you may make under `docs/` is toggling the `[ ]`/`[x]` **marker** of task/acceptance items in `docs/impl/<version>-*.md`. Never reorder, reword, add, or delete a task/acceptance line.
- Do not flip the milestone frontmatter `status` or the README table — that is `/update`'s domain (spawns `spec-curator`).
...
## What you MAY do (read-only triage + checkbox toggles)
