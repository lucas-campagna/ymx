...
- `docs/impl/README.md` — the milestone table (Version / Title / Crate(s) / File / Depends on).
...
- **Plan edits.** When spawned to add a new milestone, create `docs/impl/<version>-*.md` with the agreed frontmatter (version, title, short, description, depends_on, status: planned, tags) and add its row to the README table with its **Crate(s)** (project info; do not write agent names into docs).
