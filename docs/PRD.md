...
- **Component**: each top-level key-value pair in a document defines a component. The key is the component's name and the value gives its content (rule 2).
...
- **Template component**: a component whose name starts with `$` (e.g. `$box`). Templates are applied automatically after the component that uses them is called (rule 9). Templates can chain indefinitely (`$a`, `$$a`, `$$$a`, …).
...
> If a component has both `from` and a matching `$template`, the template chain applies FIRST, then `from` resolves against the template's result (see rule 1).
...
> If the (resolved) `from` value does **not** name a valid *regular* component, `from` is treated as a plain property — no call is made and no error is raised. Template names (those starting with `$`) are not valid `from` targets; templates are only reached through the automatic chain (rule 9).
...
### 5. Components can be called inline using `$`
...
> `$name(...)` unconditionally calls the component `name`, even if a `name` argument is in scope. Use `$name(...)` to bypass the argument lookup that bare `$name` performs (rule 3). Inline `$comp(...)` calls run during step 1 of rule 1 — before templates (rule 9) and before `from` dispatch (rule 4).

### 6. Positional arguments are supported with `$0`, `$1`, `$2`, …
...
> Inside `${...}`, bare identifiers refer to arguments (or the math result `last`) — there is **no component fallback** as there is for bare `$name` outside math (rule 3). To call a component inside a math expression, use the `name(...)` form (no `$` prefix), as in `b(12,34)` above.
...
> The shortcut fires during step 2 of rule 1 — against the post-template property set. The shortcut is **suppressed** when the component has a `from` property pointing to a valid regular component: in that case `from` is the call directive, and the otherwise-matching property is passed as a regular argument to the `from`-targeted component. The shortcut applies inside nested mini-components under the same conditions, including the same suppression when a nested `from` is valid. Suppression does **not** happen when `from` is invalid (plain property per rule 4); in that case the shortcut fires normally.
...
### 9. Components can have template components
...
> Templates can only be reached through their **direct** child: `a` invokes `$a`; if `$a` is absent, `a` does **not** skip to `$$a` — the chain is broken at that point. A template name (starting with `$`) is not a valid `from` target. Template application is step 2 of rule 1 — it sits between property resolution (step 1, where inline `$comp(...)` calls run) and `from` dispatch (step 3).

When a component's result is a scalar (not an object with named properties), it is passed to the next template as the positional argument `$0`, consistent with rule 6.
...
### 10. Non-existing properties in the calling component are ignored
...
### 11. All referenced properties are required

Any property referenced by a component (via `$name`, `$0`, `${name}`, etc.) must be supplied when the component is called. Unknown/extra properties are ignored per rule 10.
...
- A **non-array `$a`** applied to a **non-array `a`** simply calls `$a` with `a` as `$0` (per rule 9's chain semantics).

### 15. `map` and `reduce` operations via `$map` and `$reduce`

`$map`, `$reduce`, and `$merge` (rule 16) are **special forms**: each declares its own argument-evaluation strategy, rather than uniformly receiving all arguments pre-evaluated. `$map` and `$reduce` keep their first argument unevaluated (a callable component) and evaluate the array argument eagerly; `$merge` evaluates both arguments eagerly.
...
### 16. Merging objects and arrays with `$merge`
