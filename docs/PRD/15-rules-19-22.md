...
**Tag resolution.** A `from` property names a component or a literal HTML tag. If `from` is absent the default tag is `div`. A `from` value enclosed in angle brackets (e.g. `<main>`) is a literal tag name — the renderer uses it directly without component lookup. Otherwise, the component is looked up in the global namespace (per `plain` mode).

**Shorthand tag syntax (`<name>: value`).** An object key matching the `<name>` pattern (angle-bracket-enclosed identifier) is treated as a tag shortcut — equivalent to `from: <name>` with the value as `children`. The renderer detects a single non-attribute key matching `<name>` and constructs `{from: "<name>", children: <value>}` before rendering.
